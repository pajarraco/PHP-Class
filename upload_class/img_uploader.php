...
class ImgUploader
{
	...
	public $path_imgs = '../image/productos'; //path where to store images
	public $path_thumbs = '../image/productos'; //path where to store thumb images // NULL for not create 
	...
	function ImgUploader() { //check if folders are Writable or not //please CHOMD them 777
		if ((isset($this->path_original)) && (!is_writeable($this->path_original))){
   			die("Error: The directory <b>($this->path_original)</b> is NOT writable");
		}
		if ((isset($this->path_thumbs)) && (!is_writeable($this->path_thumbs))){
   			die("Error: The directory <b>($this->path_thumbs)</b> is NOT writable");
		}
		if (!is_writeable($this->path_imgs)){
   			die("Error: The directory <b>($this->path_imgs)</b> is NOT writable");
		}
	}
	...
	function img_uploader($file = ""){
		...
		$file_tmp = $file["tmp_name"];
       	...
       	if(!is_uploaded_file($file_tmp)){//check if you have selected a file.
        	die ("Error: Selecciona una imagen a subir. <br>--<a href=\"$_SERVER[PHP_SELF]\">back</a>");
		}
		...
		if (($this->extlimit == "yes") && (!in_array($file_ext,$this->limitedext))) {//check file extension
        	die ("Por favor solo extensiones de imagenes.  <br>--<a href=\"$_SERVER[PHP_SELF]\">back</a>");
		}
		...
		$ImgWidth = $this->img_width;
       	...
       	if($file_size){
			...
			$rand_name_thumb = $rand_name."_thumb"; // name for thumb file
